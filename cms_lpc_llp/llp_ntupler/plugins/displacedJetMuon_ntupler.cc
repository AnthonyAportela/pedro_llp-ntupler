...
void displacedJetMuon_ntupler::resetBranches()
{
    ...
    SaveThisEERechit.clear();
    SaveThisHCALRechit.clear();
...
void displacedJetMuon_ntupler::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  if(checkNMuonClusters(iEvent,iSetup)){
...
bool displacedJetMuon_ntupler::checkNMuonClusters(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  vector<Point> points;
	...
	if (cscchamber) {
	  ...
	  p.t = cscRechit.tpeak();
    	  p.twire = cscRechit.wireTime();
	  p.station = endcap *CSCDetId::station(cscdetid);
	  p.chamber =  endcap * (CSCDetId::station(cscdetid)*10 + CSCDetId::ring(cscdetid));
    	  p.superlayer = 0;
	...
	if (dtchamber && layer) {
	  ...
	  p.phi = globalPositionFromLayer.phi();
	  p.eta = globalPositionFromLayer.eta();
	  p.x = globalPositionFromLayer.x();
	  p.y = globalPositionFromLayer.y();
	  p.z = globalPositionFromLayer.z();
	  p.superlayer = dtlayerdetid.superlayer();
	  p.t = dtRechit.digiTime();
	  p.station = dtdetid.station();
	  p.chamber = dtdetid.wheel();
	  p.clusterID = UNCLASSIFIED;
...
bool displacedJetMuon_ntupler::fillElectrons(const edm::Event& iEvent)
{
  ...
  for (uint i = 0; i < electrons->size(); ++i){
    ...
    {
      ele_PassConvVeto[nElectrons] = true; //temp
      //ele_PassConvVeto[nElectrons] = !ConversionTools::hasMatchedConversion(ele,(*conversions),beamSpot->position()); //deprecated!
    } else {
...
bool displacedJetMuon_ntupler::fillHitsTracksAndPFCands(const edm::EventSetup& iSetup)
{
  edm::ESHandle<CaloGeometry> geoHandle;
  iSetup.get<CaloGeometryRecord>().get(geoHandle);
  const CaloSubdetectorGeometry *barrelGeometry = geoHandle->getSubdetectorGeometry(DetId::Ecal, EcalBarrel);
